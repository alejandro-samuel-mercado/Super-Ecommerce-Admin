...
export function BranchDialog({ open, onOpenChange, branch, onSuccess }: BranchDialogProps) {
  ...
  const loadCandidateUsers = useCallback(async () => {
    try {
        ...
        users = users.filter((u: { roleId: number }) => u.roleId !== 8)

        if (currentUser?.role?.name === 'ADMIN') {
            users = users.filter((u: { roleId: number }) => u.roleId === 7)
        } else if (currentUser?.role?.name === 'SUPER_ADMIN') {
            users = users.filter((u: { roleId: number }) => u.roleId === 6 || u.roleId === 7)
        } else {
             users = users.filter((u: { roleId: number }) => u.roleId === 7)
        }
  ...
  const filteredCandidates = candidateUsers.filter(user => {
  ...
  const handleAddUser = async () => {
      ...
      try {
          ...
          toast({ 
              ...
              description: error.response?.data?.message || "No se pudo asignar el usuario",
              variant: "destructive" 
          })
      } finally {
          setUsersLoading(false)
      ...
      try {
          ...
          loadBranchUsers()
           ...
           toast({ 
              ...
              description: error.response?.data?.message || "No se pudo quitar el usuario",
              variant: "destructive" 
          })
      } finally {
          setUsersLoading(false)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        <DialogHeader className="px-6 py-4 border-b border-border">
          <DialogTitle className="text-foreground">{branch ? `Editar Branch: ${branch.name}` : "Nueva Branch"}</DialogTitle>
                ...
                <TabsContent value="details" className="mt-0 h-full">
                    <form id="branch-form" onSubmit={handleSubmit} className="grid gap-4">
                        ...
                        <div className="flex items-center justify-between space-x-2 border border-border p-3 rounded-md bg-muted/20">
                            <Label htmlFor="isActive" className="flex flex-col space-y-1 cursor-pointer">
                                <span>Activa</span>
                                <span className="font-normal text-xs text-muted-foreground">Desactivar para ocultarla temporalmente.</span>
                            </Label>
                            <Switch
                            id="isActive"
                        ...
                        <div className="flex items-center justify-between space-x-2 border border-border p-3 rounded-md bg-muted/20">
                            <Label htmlFor="isHeadquarters" className="flex flex-col space-y-1 cursor-pointer">
                                <span>Sede Central</span>
                                <span className="font-normal text-xs text-muted-foreground">Marca si esta es la casa matriz.</span>
                            </Label>
                            <Switch
                            id="isHeadquarters"
                            checked={formData.isHeadquarters}
                            onCheckedChange={(c) => setFormData({ ...formData, isHeadquarters: c })}
                ...
                <TabsContent value="users" className="mt-0 space-y-4">
                    <div className="flex flex-col gap-2 bg-muted/30 p-4 rounded-lg border border-border">
                         ...
                         <div className="flex gap-2">
                             ...
                             <Button onClick={handleAddUser} disabled={!selectedUserId || usersLoading} className="hover:cursor-pointer">
                                {usersLoading ? <Loader2 className="animate-spin" /> : <Plus className="mr-2 h-4 w-4" />}
                                Asignar
                            </Button>
                         </div>
                                 ...
                                 {selectedUserId && !searchQuery ? (
                                     <div className="p-2 bg-secondary/10 border-l-4 border-secondary flex justify-between items-center">
                                         <span className="text-sm font-medium text-foreground">
                                             {candidateUsers.find(u => u.id.toString() === selectedUserId)?.name}
                                         </span>
                                         <Button variant="ghost" size="sm" onClick={() => setSelectedUserId("")} className="h-6 w-6 p-0 hover:bg-muted hover:cursor-pointer">
                                             <X className="h-3 w-3" />
                                         </Button>
                                     </div>
                                         ...
                                         {filteredCandidates.map(user => (
                                             <div 
                                                 ...
                                                 className={cn(
                                                     ...
                                                     selectedUserId === user.id.toString() && "bg-muted"
                                                 )}
                                                 onClick={() => {
                                             ...
                                             >
                                                 ...
                                                 {user.role && <Badge variant="outline" className="text-[10px]">{user.role.name}</Badge>}
                                             </div>
                                         ))}
                                     </div>
                                 ) : (
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Usuarios Asignados ({branchUsers.length})</h3>
                        </div>
                        ...
                        {usersLoading && branchUsers.length === 0 ? (
                            ...
                            <div className="grid gap-2 max-h-[300px] overflow-y-auto">
                                {branchUsers.filter(u => {
                                    ...
                                    <div key={user.id} className="flex items-center justify-between p-3 border border-border rounded-lg bg-card text-card-foreground shadow-sm">
        ...
        <DialogFooter className="px-6 py-4 border-t border-border bg-muted/30 mt-auto gap-2 sm:gap-0">
             {activeTab === 'details' ? (
                <Button className="hover:cursor-pointer" type="submit" form="branch-form" disabled={loading}>
                    {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                    Guardar Cambios
                </Button>
             ) : (
                 <Button variant="outline" onClick={() => onOpenChange(false)} className="hover:cursor-pointer border-border hover:bg-muted">
                     Cerrar
                 </Button>
             )}
        </DialogFooter>
