...
export function UserDetails({ user: initialUser, onClose }: UserDetailsProps) {
    ...
    const loadComments = useCallback(async () => {
        ...
        try {
            ...
            const userComments = allComments.filter((c: any) => c.userId === currentUser.id)
    ...
    const handleUpdateSuccess = async (updatedData: Partial<User>) => {
        try {
            if (currentUser.id) {
                const freshUser = await UsersAPI.getOne(currentUser.id)
                if (freshUser?.data) setCurrentUser(freshUser.data)
                else if (freshUser) setCurrentUser(freshUser as User)
                else setCurrentUser({ ...currentUser, ...updatedData } as User)
            } else {
    ...
    const transfers = sales.filter(s => s.paymentType === 'CASH' || s.paymentType === 'TRANSFER')
    ...
    if (isEditing) {
        return (
            <UserForm
                ...
                user={currentUser}
                currentUserRole={useAuthStore.getState().user?.role?.name as any || 'EMPLOYEE'}
                onSave={async (data) => {
            ...
            <div className="relative w-full bg-primary/30 border-b border-border p-8 shrink-0 overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6">
                    <div className="relative shrink-0">
                        ...
                        <div className={`absolute -bottom-2 -right-2 h-6 w-6 rounded-full border-4 border-card shadow-lg ${currentUser.status === 'ACTIVE' ? 'bg-emerald-500 shadow-emerald-500/50' : 'bg-destructive shadow-destructive/50'}`}></div>
                    </div>

                    <div className="flex-1 text-center md:text-left flex flex-col justify-center">
                        <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 mb-2">
                            <h2 className="text-3xl font-bold text-gray-200 tracking-tight">{currentUser.name}</h2>
                            <div className="flex flex-wrap justify-center md:justify-start gap-2">
                                <Badge variant="outline" className="bg-background/50 backdrop-blur-sm text-muted-foreground">
                                    ID: {currentUser.id}
                                </Badge>
                                <Badge variant="outline" className="border-indigo-500/30 bg-indigo-500/10 text-gray-200 uppercase">
                                    {currentUser.role?.name || 'ROL DESCONOCIDO'}
                                </Badge>
                                {currentUser.isGuest && (
                                    <Badge variant="secondary" className="bg-purple-500/20 text-purple-300 border border-purple-500/30">Invitado</Badge>
                                )}
                            </div>
                        </div>
                        <p className="text-gray-200 font-medium text-lg">{currentUser.email}</p>
                    </div>
            ...
            <div className="flex-1 sm:p-6 pt-6 px-0 grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-1 space-y-6 px-4">
                    ...
                    <div className="grid sm:grid-cols-2 grid-cols-1 gap-4 px-2">
                        <div className="bg-card border border-border rounded-3xl p-5 flex flex-col items-center justify-center hover:border-border/80 transition-all shadow-sm">
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2 flex items-center gap-1.5"><Star size={12} className="text-amber-500"/> Puntos</p>
                            <p className="text-3xl font-black text-foreground drop-shadow-sm">{currentUser.points || 0}</p>
                        </div>
                    ...
                    {stats && (
                        <div className="bg-card border border-border rounded-3xl p-6 shadow-sm w-full ">
                            <h3 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2 mb-4">
                                <span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-pulse"></span> Actividad Comercial
                            </h3>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-muted/40 rounded-2xl p-4 flex flex-col gap-1.5">
                                    ...
                                    <p className="text-xl font-bold text-foreground pl-1">{sales.length}</p>
                                </div>
                                <div className="bg-muted/40 rounded-2xl p-4 flex flex-col gap-1.5">
                                    <div className="flex items-center gap-2 text-muted-foreground">
                                        <div className="bg-background p-1.5 rounded-lg shadow-sm">
                                ...
                                <div className="bg-muted/40 rounded-2xl p-4 flex flex-col gap-1.5 col-span-2">
                                    <div className="flex items-center gap-2 text-muted-foreground">
                                        ...
                                        <span className="text-[10px] font-bold uppercase tracking-wider">Última Adquisición</span>
                                    </div>
                                    <p className="text-sm font-semibold text-foreground pl-1">
                                        {stats.lastPurchase.getTime() > 0
                                            ? format(stats.lastPurchase, "d 'de' MMMM, yyyy", { locale: es })
                                            : 'Sin registro'}
                                    </p>
                                </div>
                                {stats.topProduct && (
                                    <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4 flex flex-col gap-2 col-span-2">
                                        <div className="flex items-center gap-2 text-amber-600 dark:text-amber-500">
                                            ...
                                            <span className="text-[10px] font-bold uppercase tracking-wider">Producto Favorito</span>
                                        </div>
                                        <div className="pl-1">
                                            <p className="text-sm font-bold text-foreground line-clamp-1">{stats.topProduct[0]}</p>
                                            <p className="text-[11px] text-amber-600/80 dark:text-amber-500/80 font-medium mt-0.5">{stats.topProduct[1]} unidades adquiridas</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="bg-card border border-border rounded-3xl p-6 shadow-sm space-y-6">
                        <div>
                            <h3 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4 flex items-center gap-2">
                                <span className="w-1.5 h-1.5 bg-blue-500 rounded-full"></span> Detalles Personales
                            </h3>
                            <div className="space-y-3">
                                <div className="flex items-center gap-4 p-3 rounded-2xl bg-muted/40 hover:bg-muted/60 transition-colors">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">DNI / Documento</p>
                                        <p className="text-sm font-bold text-foreground truncate">{currentUser.dni || 'No registrado'}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4 p-3 rounded-2xl bg-muted/40 hover:bg-muted/60 transition-colors">
                                    <div className="h-10 w-10 rounded-xl bg-background flex items-center justify-center shrink-0 text-muted-foreground shadow-sm">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">Teléfono</p>
                                        <p className="text-sm font-bold text-foreground truncate">{currentUser.phone || 'No registrado'}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4 p-3 rounded-2xl bg-muted/40 hover:bg-muted/60 transition-colors">
                                    <div className="h-10 w-10 rounded-xl bg-background flex items-center justify-center shrink-0 text-muted-foreground shadow-sm">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">Fecha de Registro</p>
                                        <p className="text-sm font-bold text-foreground">
                                            {currentUser.createdAt ? format(new Date(currentUser.createdAt), "d MMMM, yyyy", { locale: es }) : '-'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="pt-6 border-t border-border">
                            <h3 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-4 flex items-center gap-2">
                                <span className="w-1.5 h-1.5 bg-pink-500 rounded-full"></span> Ubicación Geográfica
                            </h3>
                            <div className="bg-muted/40 rounded-2xl p-4">
                                <div className="flex gap-3 mb-4">
                                    ...
                                    <p className="text-sm font-bold text-foreground leading-snug mt-1.5">
                                        {currentUser.address || 'Sin dirección principal registrada'}
                                    </p>
                                </div>
                                {(currentUser.city || currentUser.state || currentUser.country || currentUser.zipCode) && (
                                    <div className="pl-12 grid grid-cols-2 gap-x-4 gap-y-3">
                                        <div>
                                            <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">Ciudad</p>
                                            <p className="text-xs font-bold text-foreground">{currentUser.city || '-'}</p>
                                        </div>
                                        <div>
                                            <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">Provincia</p>
                                            <p className="text-xs font-bold text-foreground">{currentUser.state || '-'}</p>
                                        </div>
                                        <div>
                                            <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">País</p>
                                            <p className="text-xs font-bold text-foreground">{currentUser.country || '-'}</p>
                                        </div>
                                        <div>
                                            <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">C. Postal</p>
                                            <p className="text-xs font-bold text-foreground">{currentUser.zipCode || '-'}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                ...
                <div className="p-8 pb-0 shrink-0">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-bold text-foreground">Actividad del Usuario</h3>
                        <div className="text-sm text-muted-foreground">
                            Última actualización: {currentUser.updatedAt ? format(new Date(currentUser.updatedAt), "d MMM, HH:mm", { locale: es }) : '-'}
                        </div>
                    </div>
                </div>

                <Tabs defaultValue="purchases" className="flex-1 flex flex-col sm:px-8 px-2 pb-8 overflow-hidden">
                    <TabsList className="bg-muted p-1 rounded-xl border border-border self-start mb-6 w-full sm:w-auto shadow-sm">
                        <TabsTrigger value="purchases" className="px-6 data-[state=active]:bg-primary/50 data-[state=active]:text-foreground text-muted-foreground">
                            Compras {sales.length > 0 && <span className="ml-1.5 bg-secondary/10 text-secondary text-[10px] font-bold px-1.5 py-0.5 rounded-full">{sales.length}</span>}
                        </TabsTrigger>
                        <TabsTrigger value="payments" className="px-6 data-[state=active]:bg-primary/50 data-[state=active]:text-foreground text-muted-foreground">Pagos</TabsTrigger>
                      
                        <TabsTrigger value="comments" className="px-6 data-[state=active]:bg-primary/50 data-[state=active]:text-foreground text-muted-foreground">Comentarios</TabsTrigger>
                    </TabsList>

                    {/* Compras */}
                    <TabsContent value="purchases" className="flex-1 flex flex-col overflow-hidden m-0 min-h-0">
                        {/* Buscador */}
                        {sales.length > 0 && (
                        ...
                        <div className="flex-1 overflow-y-auto pr-2 space-y-3 min-h-0">
                            {loading ? (
                                ...
                                filteredPurchases.map(sale => {
                                    ...
                                    return (
                                        <div key={sale.id} className="bg-card rounded-xl border border-border hover:border-secondary/40 transition-all overflow-hidden">
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <h4 className="font-bold text-foreground text-sm">Orden #{sale.id}</h4>
                                                        <div className="flex items-center gap-2 mt-0.5">
                                                            <span className="text-xs text-muted-foreground">
                                                                {format(new Date(sale.createdAt || new Date()), "dd/MM/yyyy · HH:mm", { locale: es })} hs
                                                            </span>
                                                            {sale.items?.length > 0 && (
                                            ...
                                            {isExpanded && (
                                                <div className="border-t border-border bg-muted/20 px-4 py-3 space-y-2 animate-in slide-in-from-top-1 duration-150">
                                                    <div className="flex items-center justify-between mb-2">
                                                        <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Detalle de productos</p>
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
                                                    ...
                                                    {sale.items?.length > 0 ? (
                                                            ...
                                                            <div key={idx} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        <p className="text-xs text-muted-foreground font-mono">SKU: {item.skuCode}</p>
                                                                    </div>
                                                                </div>
                                                        ...
                                                        <p className="text-xs text-muted-foreground text-center py-2">Sin detalle de productos disponible</p>
                                                    )}
                                ...
                                <div className="h-64 flex flex-col items-center justify-center text-muted-foreground bg-card rounded-2xl border border-dashed border-border">
                                    ...
                                    <p className="font-medium">
                                        {searchQuery ? 'No se encontraron compras con ese criterio' : 'No hay compras registradas para este usuario'}
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    {/* Pagos */}
                    <TabsContent value="payments" className="flex-1 overflow-y-auto pr-2 m-0 min-h-0 space-y-4">
                        {payments.length > 0 ? (
                            payments.map(sale => (
                                <div key={'payment-' + sale.id} onClick={() => setSelectedSale(sale)} className="bg-card rounded-xl border border-emerald-200 dark:border-emerald-800 p-4 hover:border-emerald-300 transition-all cursor-pointer flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-foreground">Pago Recibido</p>
                            ...
                            <div className="h-64 flex flex-col items-center justify-center text-muted-foreground bg-card rounded-2xl border border-dashed border-border">
                                ...
                                <p className="font-medium">No hay pagos registrados para este usuario</p>
                            </div>
                        )}
                    </TabsContent>

                    {/* Transferencias */}
                    <TabsContent value="transfers" className="flex-1 overflow-y-auto pr-2 m-0 min-h-0 space-y-4">
                        {transfers.length > 0 ? (
                            transfers.map(sale => (
                                <div key={'transfer-' + sale.id} onClick={() => setSelectedSale(sale)} className="bg-card rounded-xl border border-blue-200 dark:border-blue-800 p-4 hover:border-blue-300 transition-all cursor-pointer flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-sm font-bold text-foreground">Transferencia / {sale.paymentType}</p>
                                            <p className="text-xs text-muted-foreground">Venta #{sale.id} • {format(new Date(sale.createdAt || new Date()), "d MMM", { locale: es })}</p>
                                        </div>
                                    </div>
                            ...
                            <div className="h-64 flex flex-col items-center justify-center text-muted-foreground bg-card rounded-2xl border border-dashed border-border">
                                ...
                                <p className="font-medium">No hay transferencias registradas para este usuario</p>
                            </div>
                        )}
                    </TabsContent>

                    {/* Comentarios */}
                    <TabsContent value="comments" className="flex-1 overflow-y-auto pr-2 m-0 min-h-0 space-y-4">
                        {loadingComments ? (
                            <div className="space-y-3">
                            ...
                            comments.map(comment => (
                                <div key={comment.id} className="bg-card rounded-xl border border-border p-4 hover:border-secondary/30 transition-all flex flex-col gap-3">
                                            ...
                                            <div className="flex items-center gap-0.5">
                                                {[...Array(5)].map((_, i) => (
                                                    <Star 
                                                        key={i} 
                                                        size={12} 
                                                        className={i < (comment.rating || 0) ? "fill-amber-400 text-amber-400" : "text-muted"} 
                                                    />
                                                ))}
                                            </div>
                                            <span className="text-xs text-muted-foreground">
                                                {format(new Date(comment.createdAt), "d 'de' MMM, yyyy", { locale: es })}
                                            </span>
                                        </div>
                                        <Badge variant={comment.approved ? "outline" : "secondary"} className={comment.approved ? "text-emerald-600 border-emerald-200" : "text-amber-600 border-amber-200"}>
                                            {comment.approved ? "Aprobado" : "Pendiente"}
                                        </Badge>
                                    </div>
                                    <p className="text-sm italic text-foreground">"{comment.content}"</p>
                                    {comment.product && (
                            ...
                            <div className="h-64 flex flex-col items-center justify-center text-muted-foreground bg-card rounded-2xl border border-dashed border-border">
                                ...
                                <p className="font-medium">No hay comentarios registrados para este usuario</p>
                            </div>
                        )}
                    </TabsContent>
                </Tabs>
