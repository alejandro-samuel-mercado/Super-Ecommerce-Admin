...
export default function CouponsPage() {
    ...
    const [formData, setFormData] = useState<any>({ code: '', type: 'PERCENTAGE', value: 0, maxUses: 100, active: true })
    ...
    useEffect(() => {
        if (editingCoupon) {
            ...
            setFormData({ code: '', type: 'PERCENTAGE', value: 0, maxUses: 100, active: true, minPurchase: 0, maxDiscount: 0 })
        }
    }, [editingCoupon])
    ...
    const handleSave = async () => {
        if (formData.validFrom && formData.validUntil) {
            ...
            if (end < start) {
                toast({ title: "Error de Fechas", description: "La fecha de expiración no puede ser anterior a la fecha de inicio.", variant: "destructive" })
                return
            }
        }
        ...
        try {
            if (editingCoupon) {
                 ...
                 toast({ title: "Cupón actualizado", description: "Cambios guardados." })
            } else {
                await CouponsAPI.create(formData)
                toast({ title: "Cupón creado", description: "El cupón se ha creado correctamente." })
            }
            setIsDialogOpen(false)
            loadCoupons()
        } catch (error: any) {
             const message = error.response?.data?.message || "No se pudo guardar el cupón."
    ...
    const columns: ColumnDef<any>[] = [
         ...
         { accessorKey: "type", header: "Tipo",cell: ({ row }) => <p>Porcentaje</p> },
        ...
        <div className="sm:p-8  pt-2 space-y-6 pb-40 sm:pb-20">
             <Breadcrumb className="px-2">
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem><BreadcrumbLink>Cupones</BreadcrumbLink></BreadcrumbItem>
                </BreadcrumbList>
            </Breadcrumb>
            ...
            <div className="flex items-center justify-between px-4">
                ...
                <Button variant="outline" onClick={loadCoupons} disabled={loading} title="Recargar" className="hover:cursor-pointer">
                ...
                <DialogContent className="bg-background border-4 border-secondary/60 text-foreground sm:max-w-[425px]">
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid gap-2">
                            <Label>Código</Label>
                            <Input value={formData.code} onChange={(e) => setFormData({...formData, code: e.target.value.toUpperCase()})} className="bg-background border-input" />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="grid gap-2">
                                ...
                                <Input 
                                    ...
                                    onChange={(e) => setFormData({...formData, minPurchase: e.target.value ? parseFloat(e.target.value) : 0})} 
                                    className="bg-background border-input"
                                    placeholder="Ej: 5000"
                                />
                            </div>
                            <div className="grid gap-2">
                                <Label>Tope Reintegro</Label>
                                <Input 
                                    type="number" 
                    ...
                    <DialogFooter className="flex justify-between sm:justify-between gap-3 sm:gap-0">
                        ...
                        <Button onClick={handleSave} className="bg-secondary hover:bg-secondary/90 shadow-sm text-secondary-foreground hover:cursor-pointer">
